...
def spark_seasonality_estimation(volume_df, dates_df, lookback_days):

    @udf(returnType=DoubleType())
    def calc_seasonality_spark(t, lookback_days):
        hour = t.hour
        minute = t.minute
        volume_df = volume_df_broadcast.value
        # get volume in [t-lookback_days, t) and in the same bucket (hour, minute)
        lookback_volume_df = volume_df[(volume_df.index >= t-datetime.timedelta(days=lookback_days)) 
                                       & (volume_df.index < t) 
                                       & (volume_df['hour'] == hour) 
                                       & (volume_df['minute'] == minute)]

        if len(lookback_volume_df) == 0:
            return np.NaN
    ...
    volume_df_broadcast = spark_context.broadcast(volume_df)
    dates_sdf = spark_session.createDataFrame(dates_df)

    dates_sdf = dates_sdf.withColumn('seasonality_estimation', 
                                     calc_seasonality_spark('datetime', lit(lookback_days)))

    seasonality_estimation_df = dates_sdf.toPandas()
...
def main(exchange_str, pair_str, 
    ...
    seasonality_df = spark_seasonality_estimation(volume_df, dates_df, lookback_days)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-lbt", "--lookback_days", type=int, help='loookback time in days, Default: 30', default=30)
